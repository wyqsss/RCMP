...
class DQN():
    ...
    def built_net(self):
        ...
        with tf.variable_scope('q'):                                  # evaluation network
            ...
            self.q = tf.layers.dense(self.s, self.naction, kernel_initializer=tf.random_normal_initializer(0, 0.1))

        with tf.variable_scope('q_next'):                                           # target network, not to train
            # l_target = tf.layers.dense(self.s_, 10, tf.nn.relu, trainable=False)
            q_next = tf.layers.dense(self.s_, self.naction, kernel_initializer=tf.random_normal_initializer(0, 0.1), trainable=False)
